...
class CNN(nn.Module):
    def __init__(self, in_c=3, out_c=128, num_classes=10, values=None):
        ...
        self.conv3 = nn.Conv2d(out_c, out_c * 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv4 = nn.Conv2d(out_c * 2, out_c * 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv5 = nn.Conv2d(out_c * 2, out_c * 4, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv6 = nn.Conv2d(out_c * 4, out_c * 4, kernel_size=3, stride=1, padding=1, bias=False)

        self.fc1 = nn.Linear(4 * 4 * (out_c * 4), 1024, bias=False)
        ...
        self.bn4 = nn.BatchNorm2d(out_c * 2)
        self.bn5 = nn.BatchNorm2d(out_c * 4)
        self.bn6 = nn.BatchNorm2d(out_c * 4)
                ...
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    ...
                    if grad is not None:
                        if opt.update_met == 'min_max':
                            ...
                            prob = torch.full((grad_abs.size()), 0.9).to(grad.device)
                        else:
                    ...
                    if grad_weight is not None:
                        if opt.update_met == 'min_max':
                            ...
                            prob_weight = torch.full((grad_weight_abs.size()), 0.9).to(grad_weight.device)
                        else:
                            raise NotImplementedError(opt.update_met)
                    ...
                    if grad_bias is not None:
                        if opt.update_met == 'min_max':
                            ...
                            prob_bias = torch.full((grad_bias_abs.size()), 0.9).to(grad_bias.device)
                        else:
                            raise NotImplementedError(opt.update_met)
