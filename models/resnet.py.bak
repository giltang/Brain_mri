        ...
        for name, module in self.named_modules():
            if name in self.w_idx:
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    ...
                    if grad is not None:
                        prob_weight = (grad_abs - grad_abs.min()) / (grad_abs.max() - grad_abs.min())
                        random_prob_weight = torch.rand_like(prob_weight)

                        positive_mask_weight = (grad > 0) & (random_prob_weight < prob_weight)
                        negative_mask_weight = (grad < 0) & (random_prob_weight < prob_weight)

                        updated_idx_weight = self.w_idx[name]['weight'].to(grad.device)
                        ...
                        module.weight.data.copy_(self.values.to(grad)[updated_idx_weight])

                    if grad_bias is not None:
                        prob_bias = (grad_bias_abs - grad_bias_abs.min()) / (grad_bias_abs.max() - grad_bias_abs.min())
