# brain_mri_trainer.py (lite version for fast debug)
import os
import torch
...
epochs = 1  # test run
lr = 0.01
image_size = 64  # shrink image size
...
def load_values(pot_path, dep_path):
    ...
    combined = [p - d for p in p_vals for d in d_vals]
    return torch.tensor(sorted(set(combined)), dtype=torch.float32)
...
class MemristorCNN(nn.Module):
    def __init__(self, values):
        ...
        self.fc1 = MemristorLinear(32 * (image_size // 4) * (image_size // 4), 128, values)
        self.fc2 = MemristorLinear(128, num_classes, values)
...
# ---- Datasets (subset only) ----
train_dataset_full = datasets.ImageFolder(root=os.path.join(data_root, 'Training'), transform=train_transform)
val_dataset_full = datasets.ImageFolder(root=os.path.join(data_root, 'Testing'), transform=val_transform)
train_dataset = Subset(train_dataset_full, list(range(100)))
val_dataset = Subset(val_dataset_full, list(range(50)))
