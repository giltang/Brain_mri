# brain_mri_trainer.py (with LUT-aware training + vectorized index_step)
import os
import torch
...
epochs = 1
lr = 0.05
image_size = 64  # reduced image size for speed
...
class MemristorLinear(nn.Module):
    ...
    def index_step(self, grad_output, input_cache, lr):
        with torch.no_grad():
            ...
            updated_val_exp = updated_val.unsqueeze(-1)  # [O, I, 1]
            distances = torch.abs(updated_val_exp - self.values)  # [O, I, V]
            new_idx = torch.argmin(distances, dim=-1)
